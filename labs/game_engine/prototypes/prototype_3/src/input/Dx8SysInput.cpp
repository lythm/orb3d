...
EXPORT_C_API engine::SysInput* CreateSys()
{
	return new engine::Dx8SysInput;
}

EXPORT_C_API void DestroySys(engine::SysInput* pSys)
{
...
namespace engine
{
	...
	bool Dx8SysInput::Initialize(const SysInputParameters& param)
	{
		HRESULT         hr; 
		...
		if(m_pKeyboard == NULL ||
			m_pKeyboard->Initialize(m_pDInput, param.app_handle) == false)
		{
			return false;
		}
		...
		if(m_pMouse == NULL ||
			m_pMouse->Initialize(m_pDInput, param.app_handle) == false)
		{
			return false;
		}
