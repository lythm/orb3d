...
namespace engine
{
	...
	bool MemManager::slot_find(const Slot& s1, const Slot&s2)
	{
		return s1.blockBytes < s2.blockBytes;
	}
	void* MemManager::Alloc(uint64 bytes)
	{
		Slot s;
		s.blockBytes = bytes;
...
namespace engine
{
	...
	bool MemManager::MemSlot::Init(uint64 blockBytes)
	{
	...
	void MemManager::MemSlot::Release()
	{
	...
	void* MemManager::MemSlot::Alloc(uint64 bytes)
	{
		++m_allocCount;
	...
	void MemManager::MemSlot::Free(void* pData)
	{
		++m_freeCount;
