...
namespace engine
{
	...
	void Node::LinkTo(NodePtr pParent)
	{
		if(m_pParent)
		{
			UnLink();
	...
	void Node::UnLink()
	{
		...
		if(m_pParent->m_pFirstChild == shared_from_this())
		{
	...
	NodePtr Node::GetPrevNode()
	{
		return m_pPrev;
	}
	NodePtr Node::GetNextNode()
	{
		return m_pNext;
	}
	void Node::SetPrevNode(NodePtr pNode)
	{
		m_pPrev = pNode;
	}
	void Node::SetNextNode(NodePtr pNode)
	{
		m_pNext = pNode;
	}
	void Node::LookAt(NodePtr pNode)
	{
		if(pNode == NodePtr())
			return;
